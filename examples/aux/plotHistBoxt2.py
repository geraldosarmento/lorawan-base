...
def plotar_histograma(valores, estatisticas):
    ...
    frequencia, bins, _ = plt.hist(valores, bins=30, edgecolor='black', color='lightgray') 
    plt.xlabel('SNR (dB)', fontsize=tamanhoFonte, fontname=nomeFonte)  
...
def plotar_boxplot(valores):
    ...
    # Plota os boxplots
    plt.boxplot(conjuntos_boxplot, sym='.', patch_artist=True, boxprops=dict(facecolor='lightgray'))


    plt.xlabel('Box plots',fontsize=tamanhoFonte, fontname=nomeFonte)
    plt.ylabel('SNR (dB)',fontsize=tamanhoFonte, fontname=nomeFonte)
    plt.xticks(fontsize=tamanhoFonte, fontname=nomeFonte)
    plt.yticks(fontsize=tamanhoFonte, fontname=nomeFonte)
